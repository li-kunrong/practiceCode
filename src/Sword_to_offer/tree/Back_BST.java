...
public class Back_BST {
    ...
    /**
     * 后序遍历，先左右，后中间
     * 所以先根据根节点把数组分成两部分，小于根节点为左子树，大于根节点为右子树
     * 然后递归解决，当start>=end就返回TRUE
     * @param a
     * @param start
     * @param end
     * @return
     */
    public boolean verfy(int[] a , int start, int end) {
        if (start>=end){
            return true;
        }
        ...
        while (index<end-1&& a[index]<a[end]){
            index++;
        }
        int right = index;
        while (index<end-1&&a[index]>a[end]){
        ...
        return verfy(a,start,index-1) && verfy(a,index,end-1);
    }
}
